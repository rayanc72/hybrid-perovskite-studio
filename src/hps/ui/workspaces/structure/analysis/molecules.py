...
def render_molecule_analysis(
    *,
    ...
    if polarization_option:
        ...
        pos_coords = np.array(
            [site.frac_coords for site in structure if site.species_string in pos_atoms]
        )
        neg_coords = np.array(
            [site.frac_coords for site in structure if site.species_string in neg_atoms]
        )
    ...
    if charge_analysis_option:
        ...
        uploaded = st.file_uploader(
            "Upload Bader charge analysis output (.out)", type=["out", "txt", "dat"]
        )
        if uploaded is not None:
            ...
            with colA:
                ids_a_str = st.text_input(
                    "Atom IDs – Set A", placeholder="e.g., 1, 3, 4 or 5:10", key="charge_ids_A"
                )
            with colB:
                ids_b_str = st.text_input(
                    "Atom IDs – Set B", placeholder="e.g., 2, 6:9", key="charge_ids_B"
                )
            ...
            if ids_a or ids_b:
                # Subset
                df_a = (
                    df_all[df_all["Id"].isin(ids_a)].copy()
                    if ids_a
                    else pd.DataFrame(columns=df_all.columns)
                )
                df_b = (
                    df_all[df_all["Id"].isin(ids_b)].copy()
                    if ids_b
                    else pd.DataFrame(columns=df_all.columns)
                )
                ...
                st.info(
                    "Enter one or both ID sets above to compute charge sums, means, and their differences."
                )
    ...
    if com_option:
        ...
        scale_choice = False
        df_centroids, df_distance_matrix, lattice_vectors, df_merged = get_distance_matrix(
            modified_atoms, molecules
        )
        ...
        fig = px.scatter_3d(
            ...
            y="b",
            z="c",
            text=df_centroids.index,
            color=df_centroids.index,
            opacity=0.7,
            hover_name=df_centroids.index,
        )
        fig.update_layout(
            scene=dict(xaxis_title="a", yaxis_title="b", zaxis_title="c"),
            title="Centroids 3D Plot",
            width=600,
            height=600,
        )
        ...
        if sym_part:
            try:
                symmetric_partners = find_closest_partners(
                    df_centroids, lattice_vectors, initial_threshold=1e-3, max_iterations=1000
                )
        ...
        if use_charges:
            ...
            if charge_input_mode.startswith("Per-element"):
                ...
                def _parse_pairs_1b(text: str) -> dict:
                    ...
                    for line in re.split(r"[\n;]+", text.strip()):
                        ...
                        if len(parts) >= 2:
                            ...
                            st.warning(
                                f"Incomplete entry (need index and charge): '{line.strip()}'"
                            )
                    return m
                ...
                def _parse_out_block_from_bottom(buf) -> dict:
                    ...
                    if start is None:
                        st.warning(
                            "No 'Summary of the per-atom charge analysis:' block found in the uploaded file."
                        )
                        return {}
                    ...
                    if not charges:
                        st.warning(
                            "Found the analysis block, but could not parse any per-atom charges."
                        )
                    return charges
                    ...
                    with st.expander("see extracted charges"):
                        if global_charge_map_1b:
                            ...
                            charges_df = pd.DataFrame(
                                rows, columns=["Atom Index (1-based)", "Element", "Charge (e)"]
                            )
        ...
        if st.button("Get direction"):
            ...
            for mol_index, mol_atoms in zip(molecule_indices, chosen_molecules):
                ...
                if use_charges:
                    if (
                        use_charges
                        and charge_input_mode.startswith("Per-atom")
                        and global_charge_map_1b
                    ):
                        ...
                        per_atom_charges = [
                            float(global_charge_map_1b.get(gidx + 1, 0.0)) for gidx in mol_atoms
                        ]
                        ...
                        if global_charge_map_1b and missing:
                            st.info(
                                f"[Mol {mol_index}] {len(missing)} atoms had no provided charge; defaulted to 0.0."
                            )

                        dm_vector, dipole_moment_debye, com = get_dm_direction(
                            mol_obj, charges=per_atom_charges
                        )

                    elif (
                        charge_input_mode
                        and charge_input_mode.startswith("Per-element")
                        and len(charge_map) > 0
                    ):
                        # Per-element charges, default 0 for unspecified elements
                        per_atom_charges = [
                            float(charge_map.get(str(atom.specie), 0.0)) for atom in mol_obj
                        ]
                        dm_vector, dipole_moment_debye, com = get_dm_direction(
                            mol_obj, charges=per_atom_charges
                        )
            ...
            dm_plot = plot_dipole_moment_vectors(
                direction_df, modified_atoms, chosen_molecules, camera_pos
            )
