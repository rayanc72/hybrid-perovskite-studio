...
def create_labelled_download_file(atoms, file_name, output_suffix):
    with tempfile.NamedTemporaryFile(mode="w+", suffix=".in", delete=False) as output_labelled_file:
        ...
        st.markdown(
            get_download_link(f"{os.path.splitext(file_name)[0]}{output_suffix}_labelled.in",
                              output_labelled_content), unsafe_allow_html=True)
...
def create_aims_download_file(atoms, file_name, output_suffix):
    with tempfile.NamedTemporaryFile(mode="w+", suffix=".in", delete=False) as output_file:
        ...
        st.markdown(get_download_link(f"{os.path.splitext(file_name)[0]}{output_suffix}.in",
                                      output_content), unsafe_allow_html=True)
...
# def extract_structure_file(zip_data):
#     """
#     Extracts the .in or .cif file from a given zip data.
#
#     Parameters:
#     - zip_data (bytes): Byte data of the zip file.
#
#     Returns:
#     - file_buffer (BytesIO): A file buffer object containing the content of the first .in or .cif file found.
#     """
#     with zipfile.ZipFile(BytesIO(zip_data), 'r') as zip_ref:
#         # Find all files that match the desired extensions
#         matched_files = [f for f in zip_ref.namelist() if f.endswith('.in') or f.endswith('.cif')]
#
#         # If there are matched files, extract the first one and return its content
#         if matched_files:
#             with zip_ref.open(matched_files[0], 'r') as file:
